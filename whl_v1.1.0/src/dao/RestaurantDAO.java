...
public class RestaurantDAO {
	 ...
	 * @return 조건에 만족하는 restaurant 데이터들을 RestaurantDTO 타입의 객체들로 포장 후 ArrayList에 담아
	 *         반환<br>
	 */
	...
	/**
	 * <em>admin</em> 모드에서 음식점의 리스트를 받아오는 메소드<br>
	 ...
	 *         반환<br>
	 */
	public ArrayList<RestaurantDTO> getList(int choice, int choiceCate, int choiceSort, int limit, String keyWord) {
		ArrayList<RestaurantDTO> rl = new ArrayList<RestaurantDTO>();
		// category 배열 0번값 "" 추가
		String[] category = { "", "한식", "중식", "일식", "양식", "패스트푸드", "카페/디저트" };
		String sort = "";
	 ...
	 * @return 음식점 정보와 평점, 리뷰 수를 포함한 restaurant 데이터를 RestaurantDTO로 포장하여 반환<br>
	 */
	 ...
	 * 참조하는 user_register 데이터의 is_register의 값을 'N'에서 'Y'로 수정 및 admin_comment 추가<br>
	 * 
	...
	// restaurant에 새로운 데이터를 추가하는 것과 참조한 user_register 데이터의 is_register의 값
	// 변경('N'->'Y')는 동시에 진행되어야 함
	// 각기 다른 테이블의 추가와 변경을 동시에 진행하는 것은 불가능하므로 restaurant에 새로운 데이터를 추가 후 참조한
	// user_register 데이터 변경 진행
	// 따라서 restaurant에 새로운 값 추가에 성공하면 추천한 음식점의 데이터에서 is_register의 값을 'N'에서 'Y'로 변경
	// is_register값 변경이 실패하면 결과적으로 해당 메소드는 실패이므로 추가한 restaurant(가장 마지막 데이터)를 삭제 후
	// false 리턴
	public boolean insert(RestaurantDTO newRest, String admin_comment, int register_num) {
		String sql = "insert into restaurant(restaurant_name,category_name,restaurant_address,restaurant_phone,restaurant_capacity,restaurant_close,restaurant_description) values(?,?,?,?,?,?,?)";
		try {
			ps = conn.prepareStatement(sql);
			ps.setString(1, newRest.restaurant_name);
			...
			if (ps.executeUpdate() == 1) {
				...
				if (urdao.update(6, 7, "Y", admin_comment, register_num)) {
					return true;
				} else {
	...
	/**
	 * String 타입의 날짜("yyyy-MM-dd")를 Calendar를 이용하여 요일로 변환 후 restaurant_close 데이터와
	 * 비교<br>
	 ...
	 * @return date의 값을 restaurant_close 데이터가 포함하는 경우 false<br>
	 *         그 외에는 true<br>
	 */
	...
	public boolean checkDate(String date) {
		...
		bookDate.set(date_num[0], date_num[1] - 1, date_num[2]);
	 ...
	 * @return companion_number의 값이 restaurant_capacity보다 큰 경우 false<br>
	...
	public boolean update(int choiceCol, String newData, int restaurant_id) {
		// 1. 음식점 이름/2. 카테고리/3.주소 /4. 전화번호 /5. 예약가능 인원/6.휴무일/7.음식점 설명 / 8. 좋아요 수(시연을 위해)

		String[] cols = { "", "restaurant_name", "category_name", "restaurant_address", "restaurant_phone",
				"resturant_capacity", "restaurant_close", "restaurant_description","restaurant_like_cnt" };
		String sql = "update restaurant set " + cols[choiceCol] + " = ? where restaurant_id= ?";
		try {
			ps = conn.prepareStatement(sql);
			if(choiceCol==5||choiceCol==8) {
...
//	public boolean addLike(int restaurant_id) {
//		String sql = "update restaurant set restaurant_like_cnt = restaurant_like_cnt+1 where restaurant_id=?";
//		try {
//			ps = conn.prepareStatement(sql);
//			ps.setInt(1, restaurant_id);
//			return ps.executeUpdate()==1;
