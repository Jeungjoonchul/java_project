...
public class ReplyDAO {
	 ...
	 * 조건(user_id)에 맞는 reply 데이터를 ReplyDTO로 포장하여 ArrayList에 담아 반환<br>
	 */
	public ArrayList<ReplyDTO> getList(int restaurant_id) {
		ArrayList<ReplyDTO> result = new ArrayList<ReplyDTO>();
		String sql = "select rp.reply_num,rp.reply_date,rp.reply_comment,rp.reply_score,rp.user_id,rp.restaurant_id,rp.book_num,rs.restaurant_name,b.book_schedule from reply rp join restaurant rs on rp.restaurant_id=rs.restaurant_id join book b on b.book_num=rp.book_num where rp.restaurant_id = ?";
		try {
			ps = conn.prepareStatement(sql);
